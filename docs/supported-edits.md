...
| Add a `zeebe:taskListener` to a user task | Runs as a no-op and says so in the trace — carrying a listener never breaks a model. Supplying code is opt-in: write a handler keyed `<elementId>:<eventType>` (e.g. `Review:creating`) and it runs instead. Two listeners on one element sharing a job type is an error before Run, because an activated job carries only `type` and `elementId` — there is nothing to tell them apart with. |
