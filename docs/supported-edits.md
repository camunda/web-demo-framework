...
| Add a `zeebe:taskListener` to a user task | Runs as a no-op and says so in the trace — carrying a listener never breaks a model. Supplying code is opt-in: write a handler keyed `<elementId>:<jobType>` (e.g. `Review:notify-reviewer`) and it runs instead. Element and job type, not event type, because Camunda allows several listeners on one `eventType`. Two listeners on one element sharing a job type, or a listener sharing a job type with a manually controlled task, are errors before Run — an activated job carries only `type` and `elementId`, so neither case can be routed. |
...
| Multiple AI Agent hosts **with the scripted brain** (no live brain configured) | Fully supported, the same way: `ExampleRunner` registers a scripted handler for every distinct agent job type across every process — including hosts in a called process, where the primary process has no host of its own — and passes `job.elementId` to the scripted source so it can branch per host. A single-host example never has to read it. |
...
| Selecting an element in the **modeler** tab and configuring its properties through a form-field panel instead of raw XML attributes | Fully supported: `bpmn-js-properties-panel` (+ its Zeebe provider) sits alongside the canvas and edits flow through the same `commandStack.changed` → `saveXML()` → `onChange` path as every other edit. |
| Applying a Camunda 8 connector element template (e.g. the demo's "Inventory Check" template on the order process's "Check inventory" service task) | Fully supported via `bpmn-js-element-templates`' `CloudElementTemplates*` modules, applied through a minimal "Apply connector template" picker above the canvas (this repo doesn't pull in the separate `@bpmn-io/element-template-chooser` package). The demo template pins `zeebe:taskDefinition:type` to a `Hidden`, non-editable value matching the example's existing job type, so applying it never orphans the handler already wired for that task — it only exposes the connector's input fields (warehouse URL, timeout, SKU, minimum stock) as editable form fields. |
