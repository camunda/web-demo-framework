...
A reader can change the BPMN model in the page (via the XML/handler editors
today, a visual modeler in a later task) and re-run it. This is a plain-English
map of what that's expected to do, keyed by edit, so a reader isn't left
guessing why Run is or isn't disabled.
