...
| Parallel gateway (fork and join) | ✅ Verified | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-parallel`) — the fork runs **both** branches, not one, and the join completes the instance exactly once. Asserted on the set of branch element ids that actually ran, because a fork that took a single side would look identical from the instance count alone. |
| Abstract task (`bpmn:task`) and manual task | ✅ Verified | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-passthrough`) — neither offers a job; the token passes straight through both and the instance completes with 0 incidents. |
| Intermediate **throw** event | ❌ **Not broadcast (silently)** | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-throw` + `probe-signal-catcher`) — a signal throw does not wait (the element after it runs and the process completes), but **nothing receives the signal**: a catcher already parked on `probe-thrown-signal` keeps its subscription open and stays Active. The same catcher fires immediately on an explicit `session.broadcastSignal`, so the catcher is sound and the throw is the no-op. An earlier version of this row read "✅ Verified (signal)" on the strength of a check that only asserted the token carried on — which a skipped throw also satisfies. Message, link (#1157) and escalation (#1168) throws are separately unproven or broken. `coverage-check.mjs` now asserts the failure so the row turns red the day it changes. |
| Event-based gateway | ✅ Verified | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-event-gateway`) — both a message catch and a timer arm (visible in `snapshot.messageSubscriptions` and `snapshot.timers`), correlating the message takes the message branch, and the losing timer is **cancelled** (`snapshot.timers` empty, the timer's flow not in `takenSequenceFlows`). Asserting the loser is gone is the point: without it a gateway behaving like a parallel split would pass. |
| Script task (job typed as its element id) | ✅ Verified | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-humanwork`) — a `bpmn:scriptTask` with no `zeebe:script` is offered as an ordinary job whose type is the element's own id, which is how every example's script task is handled (`jobTypeOf` in `model.ts`). |
| User task: parks until completed | ✅ Verified | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-humanwork`) — the engine parks on it and offers no job, asserted from a snapshot taken **before** anything drives the run (the shared `driveToQuiescence` helper completes open user tasks itself, which would make parking unobservable). Completing it lets the instance finish. |
| Call activity on an ordinary sequence flow | ✅ Verified | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-caller` → `probe-called-child`) — the child process starts, its job runs, and **two** instances complete. Asserting two matters: one would mean the call activity finished without the child ever running, which is exactly the ad-hoc failure in #1159 below. |
