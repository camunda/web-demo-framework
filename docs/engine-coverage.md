...
| Intermediate **throw** event | ◑ **Partial — signal only** | `tools/probe/fixtures/audited-constructs.bpmn` (`probe-throw`) — a signal throw does not wait: the element after it runs and the instance completes. **Message, link and escalation throws are unproven** — link throw is silently skipped (**#1157**) and escalation is rejected at deploy (**#1168**). The construct audit reports this as `partial` rather than verified, so one working variant can't stand in for the element. |
