...
}`,Za={id:"loan-origination",title:"Loan origination agent",blurb:"An AI agent gathers a loan case with its own tools — customer lookup, credit bureau, an underwriting policy, a status update — then every application passes through a mandatory senior-officer review before a gateway routes it to an offer or a decline. The agent advises; the process governs.",docsUrl:"https://camunda.com/orchestrate/agents/",bpmn:Na,forms:{"loan-application":Aa,"loan-senior-officer-review":Oa},seed:ct,scenarios:[{label:"Strong applicant (policy recommends approve)",variables:ct},{label:"Marginal applicant (policy recommends decline)",variables:Ga}],scriptedAgent:Va,templates:$a,tour:xa,requiredTools:["AssessApplication","UpdateApplicationStatus"],handlers:[{elementId:"QueryCustomer",standsInFor:"CRM connector — customer lookup",source:Ya},{elementId:"CreditBureauLookup",standsInFor:"REST connector — credit bureau",source:Qa},{elementId:"AssessApplication",standsInFor:"Script task — underwriting policy (FEEL)",source:qa},{elementId:"UpdateApplicationStatus",standsInFor:"REST connector — origination system",source:Ja},{elementId:"IssueLoanOffer",standsInFor:"REST connector — offer/booking system",source:Ha},{elementId:"SendDeclineNotice",standsInFor:"REST connector — notifications",source:Wa}]},Ka=`<?xml version="1.0" encoding="UTF-8"?>
...
}`,Ks={id:"plate-recognition",title:"Read a number plate from a photo",blurb:"Pick the plate's country, then a photo goes into the run, an in-browser vision model reads the number plate on the reader's own GPU, and a human confirms or corrects it before the process records the result. The vision model recommends; the BPMN process governs. No server, no API key — with no model connected it falls back to a deterministic scripted reading.",docsUrl:"https://docs.camunda.io/docs/components/modeler/forms/camunda-forms-reference/",bpmn:vs,forms:{"plate-recognition-country":Ys,"plate-recognition-confirm":js,"plate-recognition-manual":Rs},seed:{country:"auto"},imageInput:{label:"Pick a seed photo (its plate is known, so the scripted reader works offline) or upload your own — a live in-browser model reads a photo it has never seen.",seedImages:Js},scriptedVision:Hs,handlers:[{elementId:"ExtractPlate",standsInFor:"Vision model — Florence-2 <OCR> on WebGPU (in-browser)",source:Ws},{elementId:"RecordResult",standsInFor:"Script task — records the governed outcome",source:Zs}]},Xs=[os,ya,Za,ts,cs,Ks],ed=Object.assign({"./learn-error-boundary/index.ts":io,"./learn-exclusive-gateway/index.ts":mo,"./learn-message-correlation/index.ts":ho,"./learn-multi-instance-parallel/index.ts":wo,"./learn-service-task/index.ts":No,"./learn-signal-broadcast/index.ts":jo,"./learn-timer-catch-event/index.ts":Do,"./learn-user-task-form/index.ts":Vo}),nd=Object.values(ed).map(e=>e.default).sort((e,n)=>e.id.localeCompare(n.id)),We=[...Xs,...nd];function Ft(){return"/"}function td(e=location.pathname){const t=e.match(/^\/examples\/([^/]+)\/?$/);if(t)try{return{kind:"example",id:decodeURIComponent(t[1])}}catch{return{kind:"gallery"}}return{kind:"gallery"}}function rd(e=location.search){return new URLSearchParams(e).get("embed")==="1"}function pt(e){return`${Ft()}examples/${encodeURIComponent(e)}`}const ut="p";function id(){const e=new URLSearchParams(location.search),n=e.get(ut);if(!n)return!1;const t=n.replace(/[\t\n\r]/g,"");if(!t.startsWith("/")||t.startsWith("//")||t.startsWith("/\\"))return!1;e.delete(ut);try{const r=new URL(Ft(),location.href),o=new URL(t.slice(1),r);return o.origin!==location.origin?!1:(o.search=e.toString(),o.hash=location.hash,history.replaceState(null,"",o),!0)}catch{return!1}}function od(e,n={}){const t=new URL(location.href);t.pathname=e,t.search=n.search??t.search,n.hash!==void 0&&(t.hash=n.hash),n.replace?history.replaceState(history.state,"",t):history.pushState(history.state,"",t),window.dispatchEvent(new PopStateEvent("popstate"))}function gt(){return{route:td(),embed:rd()}}function ad(){const[e,n]=h.useState(gt);return h.useEffect(()=>{const t=()=>n(gt());return window.addEventListener("popstate",t),()=>window.removeEventListener("popstate",t)},[]),e}const sd="web-demo-framework:height",dd="web-demo-framework:request-height";function ld(e){return{type:sd,height:Math.ceil(e)}}const ht="embed-height-auto";function cd(e){h.useEffect(()=>{if(!e||typeof window>"u"||window.parent===window)return;const n=document.documentElement;n.classList.add(ht);let t=-1;const r=(d=!1)=>{const p=document.documentElement.scrollHeight;!d&&Math.abs(p-t)<2||(t=p,window.parent.postMessage(ld(p),"*"))},o=d=>{if(d.source!==window.parent)return;const p=d.data;!p||p.type!==dd||r(!0)};window.addEventListener("message",o),r();const s=new ResizeObserver(()=>r());return s.observe(n),()=>{s.disconnect(),window.removeEventListener("message",o),n.classList.remove(ht)}},[e])}const bt={headline:"The model *runs*. The code is *yours* to edit.",lede:"Every example on this page is a real BPMN process executing in your browser on the nano WebAssembly engine — edit the model, edit the handlers, swap the LLM, and run it again.",tagline:"Runnable Camunda examples"};function md({text:e}){return i.jsx(i.Fragment,{children:e.split(/\*([^*]+)\*/g).map((n,t)=>t%2===1?i.jsx("em",{children:n},t):i.jsx(h.Fragment,{children:n},t))})}function pd(){const{route:e,embed:n}=ad(),t=Ct().brain,r=Ui();cd(n);const o=e.kind==="example"?e.id:We[0].id,s=We.find(u=>u.id===o)??We[0],d=We.filter(u=>u.group!=="learn-bpmn"),p=We.filter(u=>u.group==="learn-bpmn"),g=u=>{od(pt(u),{hash:location.hash})},a=s.hero??bt,l=i.jsxs(i.Fragment,{children:[!n&&i.jsxs(i.Fragment,{children:[i.jsxs("section",{className:"hero",children:[i.jsx("h1",{children:i.jsx(md,{text:a.headline})}),a.lede&&i.jsx("p",{children:a.lede})]}),i.jsx("nav",{className:"example-picker","aria-label":"Scenario examples",children:d.map(u=>i.jsx(J,{size:"sm",variant:u.id===s.id?"default":"secondary","aria-current":u.id===s.id?"page":void 0,onClick:()=>g(u.id),children:u.title},u.id))}),p.length>0&&i.jsxs(i.Fragment,{children:[i.jsx("h2",{className:"example-group-heading",id:"learn-bpmn-heading",children:"Learn BPMN"}),i.jsx("nav",{className:"example-picker","aria-labelledby":"learn-bpmn-heading",children:p.map(u=>i.jsx(J,{size:"sm",variant:u.id===s.id?"default":"secondary","aria-current":u.id===s.id?"page":void 0,onClick:()=>g(u.id),children:u.title},u.id))})]})]}),i.jsxs("div",{className:"example-meta",children:[s.docsUrl&&i.jsx("a",{className:"docs-link",href:s.docsUrl,target:"_blank",rel:"noreferrer noopener",children:"View on camunda.com ↗"}),n&&i.jsx("a",{className:"open-full-page",href:pt(s.id)+(location.hash||""),target:"_top",rel:"noreferrer",children:"Open full page ↗"})]}),i.jsx(Xi,{example:s,initialBrainKind:t,initialTourId:r},s.id)]});return n?i.jsx("div",{className:"c4-ui app-shell app-embed",children:i.jsx("main",{id:"main",className:"layout layout-embed",children:l})}):i.jsxs("div",{className:"c4-ui app-shell",children:[i.jsx(Kt,{className:"topbar",logo:i.jsx("span",{className:"brand-dot","aria-hidden":!0}),appName:"Runnable Camunda examples",trailing:i.jsx("span",{className:"app-subtitle",children:a.tagline??bt.tagline})}),i.jsx("main",{id:"main",className:"layout",children:l}),i.jsx("footer",{className:"footer",children:"Running locally in your browser on the nano WebAssembly BPMN engine — no cluster, no server, no data leaving the page."})]})}id();Gt.createRoot(document.getElementById("root")).render(i.jsx(h.StrictMode,{children:i.jsx(Xt,{children:i.jsx(pd,{})})}));export{le as _,bd as c};
