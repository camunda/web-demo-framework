...
`).replace(/\\"/g,'"').replace(/&quot;/g,'"').replace(/&#39;/g,"'").trim(),type:o[3]??"string"}))}return t}function xr(e){const n={};for(const t of St(e,"input")){const r=t.getAttribute("target");r&&(n[r]=t.getAttribute("source")??"")}return n}function Nr(e){return Array.from(e.getElementsByTagNameNS(Se,"adHocSubProcess")).filter(n=>(Ln(n)??"").startsWith(_r))}const Er=new Set(["subProcess","adHocSubProcess","callActivity"]),kr=new Set(["adHocSubProcess","subProcess","transaction"]);function Ir(e){let n=e.parentElement;for(;n;){if(n.namespaceURI===Se&&kr.has(n.localName))return n;n=n.parentElement}return null}function Tr(e,n){const t=xr(e),r=Number((t["data.limits.maxModelCalls"]??"").replace(/^=/,""));return{elementId:e.getAttribute("id")??"agent",label:e.getAttribute("name")??"Agent",jobType:Ln(e),systemPrompt:nt(t["data.systemPrompt.prompt"]),userPrompt:nt(t["data.userPrompt.prompt"]),maxModelCalls:Number.isFinite(r)&&r>0?r:10,tools:n}}function jr(e,n){var h;const t=e.getAttribute("id")??"",r=e.getAttribute("name")??t,o=Nr(e);o.length>1&&n.push({severity:"warning",elementId:o.map(p=>p.getAttribute("id")).join(", "),message:`Process "${r}" hosts ${o.length} AI Agent sub-processes (${o.map(p=>p.getAttribute("id")).join(", ")}). Each gets its own independent agent state (turn counter, called tools) — the run itself is shared across hosts, but each host's agent state within it is not.`});const s=[],d=new Map(o.map(p=>[p,[]]));for(const p of Array.from(e.getElementsByTagName("*"))){if(p.namespaceURI!==Se||o.includes(p))continue;const b=p.getAttribute("id");if(!b)continue;const _=Ir(p),N=_&&o.includes(_)?_:null;if(N&&Er.has(p.localName)){const M=p.getAttribute("name")??b,x=et(p);s.push({elementId:b,label:M,jobType:"",documentation:x,isTool:!0,compound:!0}),d.get(N).push({elementId:b,label:M,jobType:"",documentation:x,args:Mr(p),compound:!0});continue}const v=Ln(p);if(!v)continue;const f={elementId:b,label:p.getAttribute("name")??b,jobType:v,documentation:et(p),isTool:N!=null};s.push(f),N&&d.get(N).push({elementId:b,label:f.label,jobType:v,documentation:f.documentation,args:vr(p)})}const c=o.map(p=>Tr(p,d.get(p))),m=Array.from(e.getElementsByTagNameNS(Se,"userTask")).map(p=>{var b;return{elementId:p.getAttribute("id")??"",label:p.getAttribute("name")??p.getAttribute("id")??"",formId:((b=Pn(p,"formDefinition")[0])==null?void 0:b.getAttribute("formId"))??void 0}}),a=e.getElementsByTagNameNS(Se,"startEvent")[0],u=a?((h=Pn(a,"formDefinition")[0])==null?void 0:h.getAttribute("formId"))??void 0:void 0;return{processId:t,processName:r,tasks:s,agents:c,userTasks:m,startFormId:u}}function Sr(e,n={}){const t=new DOMParser().parseFromString(e,"application/xml"),r=t.getElementsByTagName("parsererror")[0];if(r)throw new Error(`Invalid BPMN XML: ${r.textContent}`);const o=Array.from(t.getElementsByTagNameNS(Se,"process"));if(o.length===0)throw new Error("No <bpmn:process> in the diagram.");const s=[],d=o.map(m=>jr(m,s));let c=n.processId?d.find(m=>m.processId===n.processId):void 0;return n.processId&&!c&&s.push({severity:"warning",message:`Requested process "${n.processId}" not found — falling back to "${d[0].processId}".`}),c??(c=d[0]),d.length>1&&s.push({severity:"warning",message:`Diagram has ${d.length} <bpmn:process> elements (${d.map(m=>m.processId).join(", ")}); using "${c.processId}" as the active process. Pass a processId to parseModel to target another.`}),{processes:d,diagnostics:s,processId:c.processId,processName:c.processName,tasks:c.tasks,agent:c.agents[0]??null,agents:d.flatMap(m=>m.agents),userTasks:c.userTasks,startFormId:c.startFormId}}function Pr(e){return e?e.imageId?{imageId:e.imageId}:e.imageName?{imageName:e.imageName}:{}:{}}function Dt(e,n){return n?e.pixels:e.imageId??e.pixels}const Ar="No image selected — pick or upload a photo to read.";function tt(){return Ar}function Dr(e,n){return async t=>{const r=e.resolve(n);if(!r)return tt();const o=Dt(r,e.live);if(o===void 0)return tt();try{return await e.read(o,t)}catch(s){return`Couldn't read the image (${s instanceof Error?s.message:String(s)}).`}}}function Cr(e,n){return async()=>{const t=e.resolve(n);if(t)return Dt(t,e.live)}}function Br(){return`<!doctype html><html><head><meta charset="utf-8"></head><body><script>
    ...
    post({ kind: "ready" });
  <\/script></body></html>`}function Ct(e,n={}){const{timeoutMs:t=5e3,onTrace:r,onVision:o,onImage:s}=n,d=`${Date.now()}-${Math.random().toString(36).slice(2)}`;return new Promise((c,m)=>{const a=document.createElement("iframe");a.setAttribute("sandbox","allow-scripts"),a.style.display="none",a.setAttribute("aria-hidden","true");let u=!1,h;const p=()=>{h&&clearTimeout(h),window.removeEventListener("message",_),a.remove()},b=v=>{u||(u=!0,p(),v())};function _(v){var M;if(v.source!==a.contentWindow)return;const f=v.data;if(!(!f||typeof f!="object")){if(f.kind==="ready"){const x=e.job,V=e.kind==="run-handler"?{kind:"run-handler",id:d,source:e.source,job:x,hasVision:e.hasVision}:{kind:"run-agent",id:d,source:e.source,job:x};(M=a.contentWindow)==null||M.postMessage(V,"*");return}"id"in f&&f.id!==d||(f.kind==="trace"?r==null||r(f.text):f.kind==="vision-request"?N(f.callId,o,"vision",f.prompt):f.kind==="image-request"?N(f.callId,s,"image"):f.kind==="result"?b(()=>c(f.value)):f.kind==="error"&&b(()=>m(new Error(f.message))))}}function N(v,f,M,...x){const V=H=>{var Y;return(Y=a.contentWindow)==null?void 0:Y.postMessage(H,"*")};if(!f){V({kind:"helper-error",id:d,callId:v,message:`${M} helper is not available.`});return}Promise.resolve().then(()=>f(...x)).then(H=>V({kind:"helper-result",id:d,callId:v,value:H}),H=>V({kind:"helper-error",id:d,callId:v,message:H instanceof Error?H.message:String(H)}))}window.addEventListener("message",_),h=setTimeout(()=>{b(()=>m(new Error(`Handler timed out after ${t}ms — the sandboxed run was terminated.`)))},t),a.srcdoc=Br(),document.body.appendChild(a)})}function Bt(e){return{key:e.key,type:e.type,elementId:e.elementId,instanceKey:e.instanceKey,variables:e.variables??{}}}function Lr(e,n,t){const r=typeof t.vision=="function";return Ct({kind:"run-handler",source:e,job:Bt(n),hasVision:r},{onTrace:t.trace,onVision:t.vision?o=>t.vision(o):void 0,onImage:t.image?()=>t.image():void 0})}function Rr(e,n){return Ct({kind:"run-agent",source:e,job:Bt(n)})}function Lt(e,n){try{new Function(`"use strict"; return (${e});`)}catch{throw new Error(`${n} has a syntax error.`)}}function Fr(e){return Lt(e,"Handler code"),(n,t)=>Lr(e,n,t)}function Or(e){return Lt(e,"Agent code"),n=>Rr(e,n)}function zr(e,n,t,r){return{sleep:o=>new Promise(s=>setTimeout(s,o)),trace:o=>n({kind:"tool",text:`   ${o}`,elementId:e.elementId,turn:t}),text:(o,s="")=>{const d=e.variables[o];return typeof d=="string"?d:d==null?s:String(d)},num:(o,s=0)=>{const d=e.variables[o],c=typeof d=="number"?d:Number(d);return Number.isFinite(c)?c:s},...r?{vision:Dr(r,e.instanceKey),image:Cr(r,e.instanceKey)}:{}}}function Ur(e){if(e===void 0)return"undefined";try{return JSON.stringify(e)}catch{return"[unserializable value]"}}function $r(e,n,t,r,o){const s={},d=e.processes.flatMap(m=>m.tasks),c=new Map(d.map(m=>[m.elementId,m.label]));for(const m of d)m.compound||s[m.jobType]||(s[m.jobType]=async a=>{const u=n[a.elementId];if(!u)throw new Error(`No handler registered for ${a.elementId} (job type ${a.type})`);const h=c.get(a.elementId)??a.elementId,p=r==null?void 0:r.current;t({kind:"tool",text:`▶ ${h}`,elementId:a.elementId,turn:p});const b=await u(a,zr(a,t,p,o));return t({kind:"vars",text:`  ↳ ${Ur(b)}`,elementId:a.elementId,result:b,turn:p}),b});return s}const Gr=/\{\{\s*([A-Za-z][A-Za-z0-9_-]*)\s*\}\}/g;function en(...e){const n=Object.create(null);for(const t of e)if(t)for(const r of Object.keys(t))n[r]=t[r];return n}function Rt(e){return(e.split("/").pop()??e).replace(/\.[^./]+$/,"")}function Ft(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Vr(e){return Ft(e).replace(/"/g,"&quot;").replace(/\n/g,"&#10;").replace(/\t/g,"&#9;")}function Yr(e){return e.replace(/\\/g,"\\\\").replace(/&/g,"&amp;").replace(/"/g,"\\&#34;").replace(/\n/g,"&#10;").replace(/\t/g,"&#9;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Qr(e){return JSON.stringify(e).slice(1,-1)}function qr(e,n){const t=e.lastIndexOf("<",n),r=e.lastIndexOf(">",n);if(t<=r)return"text";const o=e.slice(t,n);if((o.match(/"/g)??[]).length%2===0)return"text";const d=o.lastIndexOf('"');return(o.slice(d+1).match(/&#34;|&quot;/g)??[]).length%2===1?"feel-literal":"attribute"}function Jr(e,n,t="xml"){const r=[],o=new Set;return{result:e.replace(Gr,(d,c,m)=>{const a=c.trim();if(!Object.prototype.hasOwnProperty.call(n,a))return o.has(a)||(o.add(a),r.push(a)),d;const u=n[a];if(t==="json")return Qr(u);const h=qr(e,m);return h==="feel-literal"?Yr(u):h==="attribute"?Vr(u):Ft(u)}),unresolved:r}}function Hr(){return{processes:[],diagnostics:[],processId:"",processName:"",tasks:[],agent:null,agents:[],userTasks:[],startFormId:void 0}}function Wr(e,n={},t=e.bpmn,r={}){const o=[],s=en(e.templates,r),{result:d,unresolved:c}=Jr(t,s,"xml");for(const f of c)o.push({severity:"warning",message:`Template placeholder "{{${f}}}" has no matching prompt/template content — left in the model as-is, not substituted.`});let m;try{m=Sr(d)}catch(f){return o.push({severity:"error",message:f instanceof Error?f.message:String(f)}),{resolvedBpmn:d,model:Hr(),handlers:{},forms:{},diagnostics:o,hasErrors:!0}}o.push(...m.diagnostics);const a=m.processes.flatMap(f=>f.tasks),u=new Map(e.handlers.map(f=>[f.elementId,f.source])),h={};for(const f of a){if(f.compound)continue;const M=n[f.elementId]??u.get(f.elementId);if(M===void 0){o.push({severity:"error",elementId:f.elementId,jobType:f.jobType,message:`No handler for "${f.label}" (${f.elementId}, job type "${f.jobType}"). Add a handler for this element, or remove it from the diagram.`});continue}try{h[f.elementId]=Fr(M)}catch(x){o.push({severity:"error",elementId:f.elementId,jobType:f.jobType,message:`"${f.label}" (${f.elementId}): handler code didn't compile — ${x instanceof Error?x.message:String(x)}`})}}const p=new Set(a.map(f=>f.elementId)),b=new Set([...u.keys(),...Object.keys(n)]);for(const f of b)p.has(f)||o.push({severity:"error",elementId:f,message:`Handler "${f}" doesn't match any element in the current diagram — likely orphaned by a rename. Rename it back, or remove the handler.`});const _={},N=e.forms??{},v=(f,M)=>{if(!f)return;const x=N[f];x?_[f]=x:o.push({severity:"error",formId:f,message:`${M} references form "${f}", which has no matching schema.`})};for(const f of m.processes){v(f.startFormId,`The start event of process "${f.processName}"`);for(const M of f.userTasks)v(M.formId,`User task "${M.label}" (${M.elementId})`)}return{resolvedBpmn:d,model:m,handlers:h,forms:_,diagnostics:o,hasErrors:o.some(f=>f.severity==="error")}}function Zr(e){const n=e.indexOf("{");if(n<0)return null;let t=0;for(let r=n;r<e.length;r++)if(e[r]==="{")t++;else if(e[r]==="}"&&(t--,t===0))try{const o=JSON.parse(e.slice(n,r+1));return typeof o=="object"&&o!==null&&!Array.isArray(o)?o:null}catch{return null}return null}function An(e,n=220){const t=e.replace(/\s+/g," ").trim();return t.length>n?`${t.slice(0,n-1)}…`:t}function rt(e){const n=e.arguments??e.args??e.parameters??e.input;return n&&typeof n=="object"&&!Array.isArray(n)?n:{}}function it(e){if(!e)return[];const n=e.tool??e.name??e.action;if(typeof n=="string"&&n.trim())return[{name:n.trim(),args:rt(e)}];const t=e.tools??e.tool_calls??e.toolset??e.actions,r=Array.isArray(t)?t:Object.values(e).find(s=>Array.isArray(s))??[],o=[];for(const s of r)if(typeof s=="string")s.trim()&&o.push({name:s.trim(),args:{}});else if(s&&typeof s=="object"){const d=s,c=d.name??d.tool??d.id??d.function;typeof c=="string"&&c.trim()&&o.push({name:c.trim(),args:rt(d)})}return o}function Kr(e){if(!e)return!1;const n=e.done??e.finished??e.complete;return typeof n=="boolean"?n:typeof n=="string"?n.toLowerCase()==="true":!1}function ot(e){const n=e.args.length?e.args.map(r=>`      ${r.name} (${r.type}) — ${r.description}`).join(`
...
character, with nothing added. When your work is finished, reply exactly
{"done": true} and no tool.`}function ei(e,n,t,r,o=[],s=[],d=!1){const c=e.userPrompt||"Carry out your task.",m=Object.entries(n).filter(([,u])=>typeof u=="string"&&u.trim().length>0).map(([u,h])=>`  ${u}: ${String(h)}`),a=[c,m.length?`Case data:
${m.join(`
