...
}`,us={id:"loan-origination",title:"Loan origination agent",blurb:"An AI agent gathers a loan case with its own tools — customer lookup, credit bureau, an underwriting policy, a status update — then every application passes through a mandatory senior-officer review before a gateway routes it to an offer or a decline. The agent advises; the process governs.",docsUrl:"https://camunda.com/orchestrate/agents/",bpmn:Oa,forms:{"loan-application":Ja,"loan-senior-officer-review":ts},seed:_t,scenarios:[{label:"Strong applicant (policy recommends approve)",variables:_t},{label:"Marginal applicant (policy recommends decline)",variables:os}],scriptedAgent:as,templates:is,tour:za,requiredTools:["AssessApplication","UpdateApplicationStatus"],handlers:[{elementId:"QueryCustomer",standsInFor:"CRM connector — customer lookup",source:ss},{elementId:"CreditBureauLookup",standsInFor:"REST connector — credit bureau",source:ds},{elementId:"AssessApplication",standsInFor:"Script task — underwriting policy (FEEL)",source:ls},{elementId:"UpdateApplicationStatus",standsInFor:"REST connector — origination system",source:cs},{elementId:"IssueLoanOffer",standsInFor:"REST connector — offer/booking system",source:ms},{elementId:"SendDeclineNotice",standsInFor:"REST connector — notifications",source:ps}]},hs=`<?xml version="1.0" encoding="UTF-8"?>
...
}`,hd={id:"plate-recognition",title:"Read a number plate from a photo",blurb:"Pick the plate's country, then a photo goes into the run, an in-browser vision model reads the number plate on the reader's own GPU, and a human confirms or corrects it before the process records the result. The vision model recommends; the BPMN process governs. No server, no API key — with no model connected it falls back to a deterministic scripted reading.",docsUrl:"https://docs.camunda.io/docs/components/modeler/forms/camunda-forms-reference/",bpmn:Rs,forms:{"plate-recognition-country":sd,"plate-recognition-confirm":Ys,"plate-recognition-manual":Xs},seed:{country:"auto"},imageInput:{label:"Pick a seed photo (its plate is known, so the scripted reader works offline) or upload your own — a live in-browser model reads a photo it has never seen.",seedImages:cd},scriptedVision:md,handlers:[{elementId:"ExtractPlate",standsInFor:"Vision model — Florence-2 <OCR> on WebGPU (in-browser)",source:pd},{elementId:"RecordResult",standsInFor:"Script task — records the governed outcome",source:ud}]},gd=[vs,La,us,_s,ks,hd],bd=Object.assign({"./learn-error-boundary/index.ts":vo,"./learn-exclusive-gateway/index.ts":Io,"./learn-message-correlation/index.ts":Po,"./learn-multi-instance-parallel/index.ts":Bo,"./learn-service-task/index.ts":Oo,"./learn-signal-broadcast/index.ts":Yo,"./learn-timer-catch-event/index.ts":Ho,"./learn-user-task-form/index.ts":aa}),fd=Object.values(bd).map(e=>e.default).sort((e,n)=>e.id.localeCompare(n.id)),We=[...gd,...fd];function qt(){return"/"}function _d(e=location.pathname){const t=e.match(/^\/examples\/([^/]+)\/?$/);if(t)try{return{kind:"example",id:decodeURIComponent(t[1])}}catch{return{kind:"gallery"}}return{kind:"gallery"}}function wd(e=location.search){return new URLSearchParams(e).get("embed")==="1"}function yd(e=location.search){return new URLSearchParams(e).get("view")==="compact"?"compact":"full"}function vd(e=location.search){return new URLSearchParams(e).get("autostart")==="1"}function yt(e){return`${qt()}examples/${encodeURIComponent(e)}`}const vt="p";function Md(){const e=new URLSearchParams(location.search),n=e.get(vt);if(!n)return!1;const t=n.replace(/[\t\n\r]/g,"");if(!t.startsWith("/")||t.startsWith("//")||t.startsWith("/\\"))return!1;e.delete(vt);try{const r=new URL(qt(),location.href),o=new URL(t.slice(1),r);return o.origin!==location.origin?!1:(o.search=e.toString(),o.hash=location.hash,history.replaceState(null,"",o),!0)}catch{return!1}}function xd(e,n={}){const t=new URL(location.href);t.pathname=e,t.search=n.search??t.search,n.hash!==void 0&&(t.hash=n.hash),n.replace?history.replaceState(history.state,"",t):history.pushState(history.state,"",t),window.dispatchEvent(new PopStateEvent("popstate"))}function Mt(){return{route:_d(),embed:wd(),view:yd(),autostart:vd()}}function Nd(){const[e,n]=g.useState(Mt);return g.useEffect(()=>{const t=()=>n(Mt());return window.addEventListener("popstate",t),()=>window.removeEventListener("popstate",t)},[]),e}const Ed="web-demo-framework:height",kd="web-demo-framework:request-height";function Id(e){return{type:Ed,height:Math.ceil(e)}}const xt="embed-height-auto";function Td(e=document){return Math.max(e.documentElement.offsetHeight,e.body.scrollHeight)}function jd(e){g.useEffect(()=>{if(!e||typeof window>"u"||window.parent===window)return;const n=document.documentElement;n.classList.add(xt);let t=-1;const r=(l=!1)=>{const c=Td();!l&&Math.abs(c-t)<2||(t=c,window.parent.postMessage(Id(c),"*"))},o=l=>{if(l.source!==window.parent)return;const c=l.data;!c||c.type!==kd||r(!0)};window.addEventListener("message",o),r();const s=new ResizeObserver(()=>r());return s.observe(n),()=>{s.disconnect(),window.removeEventListener("message",o),n.classList.remove(xt)}},[e])}const Nt={headline:"The model *runs*. The code is *yours* to edit.",lede:"Every example on this page is a real BPMN process executing in your browser on the nano WebAssembly engine — edit the model, edit the handlers, swap the LLM, and run it again.",tagline:"Runnable Camunda examples"};function Sd({text:e}){return i.jsx(i.Fragment,{children:e.split(/\*([^*]+)\*/g).map((n,t)=>t%2===1?i.jsx("em",{children:n},t):i.jsx(g.Fragment,{children:n},t))})}function Pd(){const{route:e,embed:n,view:t,autostart:r}=Nd(),o=n&&t==="compact",s=Gt().brain,l=no();jd(n);const c=e.kind==="example"?e.id:We[0].id,m=We.find(f=>f.id===c)??We[0],a=We.filter(f=>f.group!=="learn-bpmn"),u=We.filter(f=>f.group==="learn-bpmn"),h=f=>{xd(yt(f),{hash:location.hash})},d=m.hero??Nt,b=i.jsxs(i.Fragment,{children:[!n&&i.jsxs(i.Fragment,{children:[i.jsxs("section",{className:"hero",children:[i.jsx("h1",{children:i.jsx(Sd,{text:d.headline})}),d.lede&&i.jsx("p",{children:d.lede})]}),i.jsx("nav",{className:"example-picker","aria-label":"Scenario examples",children:a.map(f=>i.jsx(Z,{size:"sm",variant:f.id===m.id?"default":"secondary","aria-current":f.id===m.id?"page":void 0,onClick:()=>h(f.id),children:f.title},f.id))}),u.length>0&&i.jsxs(i.Fragment,{children:[i.jsx("h2",{className:"example-group-heading",id:"learn-bpmn-heading",children:"Learn BPMN"}),i.jsx("nav",{className:"example-picker","aria-labelledby":"learn-bpmn-heading",children:u.map(f=>i.jsx(Z,{size:"sm",variant:f.id===m.id?"default":"secondary","aria-current":f.id===m.id?"page":void 0,onClick:()=>h(f.id),children:f.title},f.id))})]})]}),i.jsxs("div",{className:"example-meta",children:[m.docsUrl&&i.jsx("a",{className:"docs-link",href:m.docsUrl,target:"_blank",rel:"noreferrer noopener",children:"View on camunda.com ↗"}),n&&i.jsx("a",{className:"open-full-page",href:yt(m.id)+(location.hash||""),target:"_top",rel:"noreferrer",children:o?"Open the editable version ↗":"Open full page ↗"})]}),i.jsx(bo,{example:m,compact:o,autostart:r,initialBrainKind:s,initialTourId:l},m.id)]});return n?i.jsx("div",{className:`c4-ui app-shell app-embed${o?" app-compact":""}`,children:i.jsx("main",{id:"main",className:`layout layout-embed${o?" layout-compact":""}`,children:b})}):i.jsxs("div",{className:"c4-ui app-shell",children:[i.jsx(mr,{className:"topbar",logo:i.jsx("span",{className:"brand-dot","aria-hidden":!0}),appName:"Runnable Camunda examples",trailing:i.jsx("span",{className:"app-subtitle",children:d.tagline??Nt.tagline})}),i.jsx("main",{id:"main",className:"layout",children:b}),i.jsx("footer",{className:"footer",children:"Running locally in your browser on the nano WebAssembly BPMN engine — no cluster, no server, no data leaving the page."})]})}Md();tr.createRoot(document.getElementById("root")).render(i.jsx(g.StrictMode,{children:i.jsx(pr,{children:i.jsx(Pd,{})})}));export{ue as _,Bd as c};
