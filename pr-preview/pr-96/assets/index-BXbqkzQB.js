...
`,jo="Camunda Cloud",Po="8.10.0",So={name:"Camunda Web Modeler",version:"9b5d5ef"},Ao=19,Do="learn-user-task-form-review",Co=[{text:`# Review request
...
**Details:** {{details}}`,type:"text",layout:{row:"Row_details",columns:null},id:"Field_ReviewDetails"},{label:"Decision",values:[{label:"Approve",value:"approved"},{label:"Reject",value:"rejected"}],type:"radio",layout:{row:"Row_decision",columns:null},id:"Field_ReviewDecision",key:"decision",validate:{required:!0}},{label:"Comments",description:"Optional note recorded alongside your decision.",type:"textarea",layout:{row:"Row_comments",columns:null},id:"Field_ReviewComments",key:"comments"}],Bo="default",Lo={executionPlatform:jo,executionPlatformVersion:Po,exporter:So,schemaVersion:Ao,id:Do,components:Co,type:Bo},Ro={id:"learn-user-task-form",title:"User task + form",group:"learn-bpmn",blurb:"A user task is a step a human completes, not a worker — the token parks at the task until someone submits its form, then moves on. Run this and watch the process reach 'Review request' and wait; fill in the decision form and submit it to see the token resume and the process reach its end event. A user task with no zeebe:formDefinition binding still deploys and still parks the token, but the runner has no form to render for it, so it offers a bare Complete button that finishes the task with no variables.",docsUrl:"https://docs.camunda.io/docs/components/modeler/bpmn/user-tasks/",bpmn:To,forms:{"learn-user-task-form-review":Lo},seed:{requester:"Priya Shah",details:"Approve access to the shared design-review workspace."},handlers:[]},zo=Object.freeze(Object.defineProperty({__proto__:null,default:Ro},Symbol.toStringTag,{value:"Module"})),Fo=`You are a demo workflow assistant for fictional compliance checks.
...
`,Uo={id:"compliance-walkthrough",label:"Take the tour",steps:[{title:"Start a run",description:"Press Run to send a shipment through the compliance agent.",target:{anchor:fe.runButton}},{title:"The agent picks its own tools",description:"This AI Agent reads the shipment notes and decides, turn by turn, which of the tools below it to call — nothing here is hard-coded into a fixed sequence.",target:{elementId:"ComplianceCheckAgent"}},{title:"Watch the token move",description:"The agent's first move is to look up the genetic marker mentioned in the notes.",target:{elementId:"VerifyGeneticMarker"},waitFor:{kind:"activeElement",elementId:"VerifyGeneticMarker"}},{title:"A cleared shipment notifies the export team",description:"Once the compliance score comes back clean, the process notifies the export team automatically — no human review needed for this scenario.",target:{elementId:"NotifyExportTeam"},waitFor:{kind:"elementCompleted",elementId:"NotifyExportTeam"}},{title:"Everything the run recorded",description:"The variables panel shows the marker record, the country lookup, the compliance score, and the final decision — exactly what each tool and the agent wrote along the way.",target:{anchor:fe.variablesPanel}}],successEvent:{kind:"instanceCompleted"}},$o=`<?xml version="1.0" encoding="UTF-8"?>
...
**Agent's decision:** {{decision}}`,type:"text",layout:{row:"Row_findings",columns:null},id:"Field_ReviewFindings"},{label:"Reviewer decision",values:[{label:"Approve for export",value:"approved"},{label:"Reject shipment",value:"rejected"}],type:"radio",layout:{row:"Row_review_decision",columns:null},id:"Field_ReviewDecision",key:"reviewDecision",validate:{required:!0}},{label:"Reviewer comments",description:"Explain your decision - this is recorded alongside the process instance.",type:"textarea",layout:{row:"Row_review_comments",columns:null},id:"Field_ReviewComments",key:"reviewComments"}],ra="default",ia={executionPlatform:Zo,executionPlatformVersion:Ko,exporter:Xo,schemaVersion:ea,id:na,components:ta,type:ra},oa=Object.assign({"./prompts/system-prompt.md":Fo,"./prompts/user-prompt.md":Oo}),aa=Qe(Object.fromEntries(Object.entries(oa).map(([e,n])=>[ut(e),n.trimEnd()]))),Jn="SHIP-2026-0731 is prepared for transfer to Brazil. Supporting paperwork includes reference code TP53",sa="SHIP-2026-0900 is prepared for transfer to Germany. Supporting paperwork includes reference code BRCA1",da=`async (job) => {
...
}`,ga={id:"seed-export-compliance",title:"Seed export compliance agent",blurb:"An AI agent picks its own tools to check a shipment, then a gateway routes on its decision — cleared shipments notify the export team, flagged ones go to a human. The LLM recommends; the BPMN process governs.",docsUrl:"https://camunda.com/blog/agentic-ai/",bpmn:$o,forms:{"seed-export-shipment-ready":Wo,"seed-export-compliance-review":ia},seed:{scenario:Jn,shipmentNotes:""},scenarios:[{label:"Likely cleared (TP53 → Brazil)",variables:{scenario:Jn,shipmentNotes:""}},{label:"Likely flagged (BRCA1 → Germany)",variables:{scenario:sa,shipmentNotes:""}}],scriptedAgent:da,templates:aa,tour:Uo,requiredTools:["RecordComplianceDecision"],handlers:[{elementId:"VerifyGeneticMarker",standsInFor:"JDBC connector — UCSC hg38",source:la},{elementId:"CheckDestinationCountry",standsInFor:"GraphQL connector — countries API",source:ca},{elementId:"ComputeComplianceScore",standsInFor:"REST connector — api.mathjs.org",source:ma},{elementId:"RecordComplianceDecision",standsInFor:"Script task — FEEL",source:pa},{elementId:"NotifyExportTeam",standsInFor:"REST connector — httpbin.io",source:ua}]},ha=`You are a loan origination assistant at a retail bank. Your job is to gather everything a senior loan officer needs to decide an application — you do **not** decide it yourself.
...
}`,Oa=`async (job, { text, num, sleep, trace }) => {
...
}`,Ya={id:"loan-origination",title:"Loan origination agent",blurb:"An AI agent gathers a loan case with its own tools — customer lookup, credit bureau, an underwriting policy, a status update — then every application passes through a mandatory senior-officer review before a gateway routes it to an offer or a decline. The agent advises; the process governs.",docsUrl:"https://camunda.com/orchestrate/agents/",bpmn:wa,forms:{"loan-application":ka,"loan-senior-officer-review":Ca},seed:Hn,scenarios:[{label:"Strong applicant (policy recommends approve)",variables:Hn},{label:"Marginal applicant (policy recommends decline)",variables:Ra}],scriptedAgent:za,templates:La,tour:_a,requiredTools:["AssessApplication"],handlers:[{elementId:"QueryCustomer",standsInFor:"CRM connector — customer lookup",source:Fa},{elementId:"CreditBureauLookup",standsInFor:"REST connector — credit bureau",source:Oa},{elementId:"AssessApplication",standsInFor:"Script task — underwriting policy (FEEL)",source:Ua},{elementId:"UpdateApplicationStatus",standsInFor:"REST connector — origination system",source:$a},{elementId:"IssueLoanOffer",standsInFor:"REST connector — offer/booking system",source:Va},{elementId:"SendDeclineNotice",standsInFor:"REST connector — notifications",source:Ga}]},Qa=`<?xml version="1.0" encoding="UTF-8"?>
...
`,qa=`async (job, { text, sleep }) => {
  ...
  return { shipped: true, tracking: "1Z" + Math.floor(Math.random() * 1e9) };
}`,Wa={id:"order-process",title:"Order process with service workers",blurb:"The getting-started order process: check inventory, charge payment, ship. No agent and no human step — the same runner, driven entirely by what's in the diagram.",docsUrl:"https://docs.camunda.io/docs/next/guides/getting-started-orchestration-cluster/",bpmn:Qa,seed:{item:"camunda-t-shirt",quantity:3},handlers:[{elementId:"Activity_0tw2fu0",standsInFor:"job worker — check-inventory",source:qa},{elementId:"Activity_1ppsbgi",standsInFor:"job worker — charge-payment",source:Ja},{elementId:"Activity_08pg6im",standsInFor:"job worker — ship-items",source:Ha}]},Za=`<?xml version="1.0" encoding="UTF-8"?>\r
...
}`,Xa={id:"rocket-launch",title:"Rocket launch",blurb:"The getting-started rocket launch, boiled down to one service task: launch. The smallest possible example, and the smallest possible test of the framework's extensibility.",bpmn:Za,seed:{missionName:"Apollo 11",destination:"the Moon",fuelLevel:100},scenarios:[{label:"Full tanks — launch succeeds",variables:{missionName:"Apollo 11",destination:"the Moon",fuelLevel:100}},{label:"Low fuel — mission scrubbed",variables:{missionName:"Apollo 13",destination:"the Moon",fuelLevel:30}}],handlers:[{elementId:"Activity_LaunchRocket",standsInFor:"job worker — launch-rocket",source:Ka}]},es=`<?xml version="1.0" encoding="UTF-8"?>\r
...
`,ns=`async (job, { text, sleep }) => {
  ...
  return { shipped: true, tracking: "1Z" + Math.floor(Math.random() * 1e9) };
}`,is={id:"order-process-boundary-events",title:"Order process with boundary events",blurb:"The getting-started order process, extended with a timer and an error boundary event: charge payment can be declined, and a delayed shipment can escalate — both fired by hand from the runner rather than by chance.",docsUrl:"https://github.com/camunda/camunda-8-get-started/tree/main/2-order-process-with-service-workers",bpmn:es,seed:{item:"camunda-t-shirt",quantity:3},handlers:[{elementId:"Activity_0tw2fu0",standsInFor:"job worker — check-inventory",source:ns},{elementId:"Activity_1ppsbgi",standsInFor:"job worker — charge-payment",source:ts,manualControl:{label:"Charge payment method",completeLabel:"✅ Complete normally",action:{kind:"error",errorCode:"CHARGE_DECLINED",message:"The card issuer declined the charge.",label:"❌ Simulate: card declined"}}},{elementId:"Activity_08pg6im",standsInFor:"job worker — ship-items",source:rs,manualControl:{label:"Ship items",completeLabel:"✅ Ship it",action:{kind:"timer",label:"🕐 Fire the shipping-delayed timer"}}}]},os="/pr-preview/pr-96/assets/de-bmw-mini-JBSk7QcF.jpg",as="/pr-preview/pr-96/assets/de-bmw-mini.thumb-CUUmJrRO.jpg",ss="/pr-preview/pr-96/assets/uk-d651-rnb-XGipy2QN.jpg",ds="/pr-preview/pr-96/assets/uk-d651-rnb.thumb-mjEcbhUf.jpg",ls="/pr-preview/pr-96/assets/uk-mk70-orj-Cn6O3Xfm.jpg",cs="/pr-preview/pr-96/assets/uk-mk70-orj.thumb-CaeZ2vqU.jpg",ms="/pr-preview/pr-96/assets/uk-ni-ijz-8992-YXV44tgk.jpg",ps="/pr-preview/pr-96/assets/uk-ni-ijz-8992.thumb-DYwok8jV.jpg",us="/pr-preview/pr-96/assets/us-hyundai-genesis-gGpAIEpi.jpg",gs="/pr-preview/pr-96/assets/us-hyundai-genesis.thumb-DEEt19Mw.jpg",hs=`<?xml version="1.0" encoding="UTF-8"?>
...
}`,Qs={id:"plate-recognition",title:"Read a number plate from a photo",blurb:"Pick the plate's country, then a photo goes into the run, an in-browser vision model reads the number plate on the reader's own GPU, and a human confirms or corrects it before the process records the result. The vision model recommends; the BPMN process governs. No server, no API key — with no model connected it falls back to a deterministic scripted reading.",docsUrl:"https://docs.camunda.io/docs/components/modeler/forms/camunda-forms-reference/",bpmn:hs,forms:{"plate-recognition-country":Fs,"plate-recognition-confirm":xs,"plate-recognition-manual":Ss},seed:{country:"auto"},imageInput:{label:"Pick a seed photo (its plate is known, so the scripted reader works offline) or upload your own — a live in-browser model reads a photo it has never seen.",seedImages:$s},scriptedVision:Vs,handlers:[{elementId:"ExtractPlate",standsInFor:"Vision model — Florence-2 <OCR> on WebGPU (in-browser)",source:Gs},{elementId:"RecordResult",standsInFor:"Script task — records the governed outcome",source:Ys}]},qs=[Xa,ga,Ya,Wa,is,Qs],Js=Object.assign({"./learn-error-boundary/index.ts":Xi,"./learn-exclusive-gateway/index.ts":oo,"./learn-message-correlation/index.ts":co,"./learn-multi-instance-parallel/index.ts":go,"./learn-service-task/index.ts":fo,"./learn-signal-broadcast/index.ts":xo,"./learn-timer-catch-event/index.ts":Io,"./learn-user-task-form/index.ts":zo}),Hs=Object.values(Js).map(e=>e.default).sort((e,n)=>e.id.localeCompare(n.id)),$e=[...qs,...Hs];function fn(){return"/pr-preview/pr-96/"}function Ws(e){const n=fn();return e.startsWith(n)?"/"+e.slice(n.length):e}function Zs(e=location.pathname){const t=Ws(e).match(/^\/examples\/([^/]+)\/?$/);if(t)try{return{kind:"example",id:decodeURIComponent(t[1])}}catch{return{kind:"gallery"}}return{kind:"gallery"}}function Ks(e=location.search){return new URLSearchParams(e).get("embed")==="1"}function Xs(){return fn()}function Zn(e){return`${fn()}examples/${encodeURIComponent(e)}`}function Kn(e,n={}){const t=new URL(location.href);t.pathname=e,t.search=n.search??t.search,n.hash!==void 0&&(t.hash=n.hash),n.replace?history.replaceState(history.state,"",t):history.pushState(history.state,"",t),window.dispatchEvent(new PopStateEvent("popstate"))}function Xn(){return{route:Zs(),embed:Ks()}}function ed(){const[e,n]=h.useState(Xn);return h.useEffect(()=>{const t=()=>n(Xn());return window.addEventListener("popstate",t),()=>window.removeEventListener("popstate",t)},[]),e}const nd="web-demo-framework:height",td="web-demo-framework:request-height";function rd(e){return{type:nd,height:Math.ceil(e)}}const et="embed-height-auto";function id(e){h.useEffect(()=>{if(!e||typeof window>"u"||window.parent===window)return;const n=document.documentElement;n.classList.add(et);let t=-1;const r=(l=!1)=>{const m=document.documentElement.scrollHeight;!l&&Math.abs(m-t)<2||(t=m,window.parent.postMessage(rd(m),"*"))},i=l=>{if(l.source!==window.parent)return;const m=l.data;!m||m.type!==td||r(!0)};window.addEventListener("message",i),r();const s=new ResizeObserver(()=>r());return s.observe(n),()=>{s.disconnect(),window.removeEventListener("message",i),n.classList.remove(et)}},[e])}function od(){const{route:e,embed:n}=ed(),t=Nt().brain,r=Ri();id(n);const i=e.kind==="example"?e.id:$e[0].id,s=$e.find(d=>d.id===i)??$e[0],l=$e.filter(d=>d.group!=="learn-bpmn"),m=$e.filter(d=>d.group==="learn-bpmn"),p=d=>{Kn(Zn(d),{hash:location.hash})},o=a.jsxs(a.Fragment,{children:[!n&&e.kind==="gallery"&&a.jsxs(a.Fragment,{children:[a.jsx("nav",{className:"example-picker",children:l.map(d=>a.jsx(H,{size:"sm",variant:d.id===s.id?"default":"secondary",onClick:()=>p(d.id),children:d.title},d.id))}),m.length>0&&a.jsxs(a.Fragment,{children:[a.jsx("h2",{className:"example-group-heading",children:"Learn BPMN"}),a.jsx("nav",{className:"example-picker",children:m.map(d=>a.jsx(H,{size:"sm",variant:d.id===s.id?"default":"secondary",onClick:()=>p(d.id),children:d.title},d.id))})]})]}),!n&&e.kind==="example"&&a.jsx("div",{className:"example-nav",children:a.jsx(H,{size:"sm",variant:"secondary",onClick:()=>Kn(Xs()),children:"← All examples"})}),a.jsxs("div",{className:"example-meta",children:[s.docsUrl&&a.jsx("a",{className:"docs-link",href:s.docsUrl,target:"_blank",rel:"noreferrer noopener",children:"View on camunda.com ↗"}),n&&a.jsx("a",{className:"open-full-page",href:Zn(s.id)+(location.hash||""),target:"_top",rel:"noreferrer",children:"Open full page ↗"})]}),a.jsx(Ji,{example:s,initialBrainKind:t,initialTourId:r},s.id)]});return n?a.jsx("div",{className:"c4-ui app-shell app-embed",children:a.jsx("main",{id:"main",className:"layout layout-embed",children:o})}):a.jsxs("div",{className:"c4-ui app-shell",children:[a.jsx(Gt,{appName:"Runnable Camunda examples",trailing:a.jsx("span",{className:"app-subtitle",children:"model + code + optional LLM, in your browser"})}),a.jsx("main",{id:"main",className:"layout",children:o})]})}Bt.createRoot(document.getElementById("root")).render(a.jsx(h.StrictMode,{children:a.jsx(Yt,{children:a.jsx(od,{})})}));export{ue as _,ld as c};
