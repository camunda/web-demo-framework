...
}`,Ja={id:"loan-origination",title:"Loan origination agent",blurb:"An AI agent gathers a loan case with its own tools — customer lookup, credit bureau, an underwriting policy, a status update — then every application passes through a mandatory senior-officer review before a gateway routes it to an offer or a decline. The agent advises; the process governs.",docsUrl:"https://camunda.com/orchestrate/agents/",bpmn:va,forms:{"loan-application":ja,"loan-senior-officer-review":Ra},seed:st,scenarios:[{label:"Strong applicant (policy recommends approve)",variables:st},{label:"Marginal applicant (policy recommends decline)",variables:Oa}],scriptedAgent:Ua,templates:za,tour:ya,requiredTools:["AssessApplication","UpdateApplicationStatus"],handlers:[{elementId:"QueryCustomer",standsInFor:"CRM connector — customer lookup",source:$a},{elementId:"CreditBureauLookup",standsInFor:"REST connector — credit bureau",source:Ga},{elementId:"AssessApplication",standsInFor:"Script task — underwriting policy (FEEL)",source:Va},{elementId:"UpdateApplicationStatus",standsInFor:"REST connector — origination system",source:Ya},{elementId:"IssueLoanOffer",standsInFor:"REST connector — offer/booking system",source:Qa},{elementId:"SendDeclineNotice",standsInFor:"REST connector — notifications",source:qa}]},Ha=`<?xml version="1.0" encoding="UTF-8"?>
...
}`,Hs={id:"plate-recognition",title:"Read a number plate from a photo",blurb:"Pick the plate's country, then a photo goes into the run, an in-browser vision model reads the number plate on the reader's own GPU, and a human confirms or corrects it before the process records the result. The vision model recommends; the BPMN process governs. No server, no API key — with no model connected it falls back to a deterministic scripted reading.",docsUrl:"https://docs.camunda.io/docs/components/modeler/forms/camunda-forms-reference/",bpmn:fs,forms:{"plate-recognition-country":$s,"plate-recognition-confirm":ks,"plate-recognition-manual":Cs},seed:{country:"auto"},imageInput:{label:"Pick a seed photo (its plate is known, so the scripted reader works offline) or upload your own — a live in-browser model reads a photo it has never seen.",seedImages:Ys},scriptedVision:Qs,handlers:[{elementId:"ExtractPlate",standsInFor:"Vision model — Florence-2 <OCR> on WebGPU (in-browser)",source:qs},{elementId:"RecordResult",standsInFor:"Script task — records the governed outcome",source:Js}]},Ws=[ts,_a,Ja,Xa,ss,Hs],Zs=Object.assign({"./learn-error-boundary/index.ts":no,"./learn-exclusive-gateway/index.ts":so,"./learn-message-correlation/index.ts":po,"./learn-multi-instance-parallel/index.ts":bo,"./learn-service-task/index.ts":vo,"./learn-signal-broadcast/index.ts":ko,"./learn-timer-catch-event/index.ts":So,"./learn-user-task-form/index.ts":Uo}),Ks=Object.values(Zs).map(e=>e.default).sort((e,n)=>e.id.localeCompare(n.id)),He=[...Ws,...Ks];function Dn(){return"/pr-preview/pr-99/"}function Xs(e){const n=Dn();return e.startsWith(n)?"/"+e.slice(n.length):e}function ed(e=location.pathname){const t=Xs(e).match(/^\/examples\/([^/]+)\/?$/);if(t)try{return{kind:"example",id:decodeURIComponent(t[1])}}catch{return{kind:"gallery"}}return{kind:"gallery"}}function nd(e=location.search){return new URLSearchParams(e).get("embed")==="1"}function lt(e){return`${Dn()}examples/${encodeURIComponent(e)}`}const ct="p";function td(){const e=new URLSearchParams(location.search),n=e.get(ct);if(!n)return!1;const t=n.replace(/[\t\n\r]/g,"");if(!t.startsWith("/")||t.startsWith("//")||t.startsWith("/\\"))return!1;e.delete(ct);try{const r=new URL(Dn(),location.href),i=new URL(t.slice(1),r);return i.origin!==location.origin?!1:(i.search=e.toString(),i.hash=location.hash,history.replaceState(null,"",i),!0)}catch{return!1}}function rd(e,n={}){const t=new URL(location.href);t.pathname=e,t.search=n.search??t.search,n.hash!==void 0&&(t.hash=n.hash),n.replace?history.replaceState(history.state,"",t):history.pushState(history.state,"",t),window.dispatchEvent(new PopStateEvent("popstate"))}function mt(){return{route:ed(),embed:nd()}}function id(){const[e,n]=h.useState(mt);return h.useEffect(()=>{const t=()=>n(mt());return window.addEventListener("popstate",t),()=>window.removeEventListener("popstate",t)},[]),e}const od="web-demo-framework:height",ad="web-demo-framework:request-height";function sd(e){return{type:od,height:Math.ceil(e)}}const pt="embed-height-auto";function dd(e){h.useEffect(()=>{if(!e||typeof window>"u"||window.parent===window)return;const n=document.documentElement;n.classList.add(pt);let t=-1;const r=(l=!1)=>{const m=document.documentElement.scrollHeight;!l&&Math.abs(m-t)<2||(t=m,window.parent.postMessage(sd(m),"*"))},i=l=>{if(l.source!==window.parent)return;const m=l.data;!m||m.type!==ad||r(!0)};window.addEventListener("message",i),r();const s=new ResizeObserver(()=>r());return s.observe(n),()=>{s.disconnect(),window.removeEventListener("message",i),n.classList.remove(pt)}},[e])}function ld(){const{route:e,embed:n}=id(),t=Pt().brain,r=zi();dd(n);const i=e.kind==="example"?e.id:He[0].id,s=He.find(d=>d.id===i)??He[0],l=He.filter(d=>d.group!=="learn-bpmn"),m=He.filter(d=>d.group==="learn-bpmn"),p=d=>{rd(lt(d),{hash:location.hash})},a=o.jsxs(o.Fragment,{children:[!n&&o.jsxs(o.Fragment,{children:[o.jsx("nav",{className:"example-picker","aria-label":"Scenario examples",children:l.map(d=>o.jsx(J,{size:"sm",variant:d.id===s.id?"default":"secondary","aria-current":d.id===s.id?"page":void 0,onClick:()=>p(d.id),children:d.title},d.id))}),m.length>0&&o.jsxs(o.Fragment,{children:[o.jsx("h2",{className:"example-group-heading",id:"learn-bpmn-heading",children:"Learn BPMN"}),o.jsx("nav",{className:"example-picker","aria-labelledby":"learn-bpmn-heading",children:m.map(d=>o.jsx(J,{size:"sm",variant:d.id===s.id?"default":"secondary","aria-current":d.id===s.id?"page":void 0,onClick:()=>p(d.id),children:d.title},d.id))})]})]}),o.jsxs("div",{className:"example-meta",children:[s.docsUrl&&o.jsx("a",{className:"docs-link",href:s.docsUrl,target:"_blank",rel:"noreferrer noopener",children:"View on camunda.com ↗"}),n&&o.jsx("a",{className:"open-full-page",href:lt(s.id)+(location.hash||""),target:"_top",rel:"noreferrer",children:"Open full page ↗"})]}),o.jsx(Wi,{example:s,initialBrainKind:t,initialTourId:r},s.id)]});return n?o.jsx("div",{className:"c4-ui app-shell app-embed",children:o.jsx("main",{id:"main",className:"layout layout-embed",children:a})}):o.jsxs("div",{className:"c4-ui app-shell",children:[o.jsx(qt,{appName:"Runnable Camunda examples",trailing:o.jsx("span",{className:"app-subtitle",children:"model + code + optional LLM, in your browser"})}),o.jsx("main",{id:"main",className:"layout",children:a})]})}td();Ft.createRoot(document.getElementById("root")).render(o.jsx(h.StrictMode,{children:o.jsx(Jt,{children:o.jsx(ld,{})})}));export{ge as _,ud as c};
