import{n}from"./index-DBMYmS0b.js";import"./vendor-react-9Ma26nY1.js";import"./vendor-design-system-jWpzD6a9.js";const e=`<?xml version="1.0" encoding="UTF-8"?>
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:zeebe="http://camunda.org/schema/zeebe/1.0" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" xmlns:modeler="http://camunda.org/schema/modeler/1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" id="Definitions_OrderBoundaryEvents" targetNamespace="http://bpmn.io/schema/bpmn" exporter="Camunda Modeler" exporterVersion="5.36.1" modeler:executionPlatform="Camunda Cloud" modeler:executionPlatformVersion="8.8.0">
  <bpmn:process id="order-process-boundary-events" name="Order process with boundary events" isExecutable="true">
    ...
    <!--
      The two error boundary events below are adapted from
      camunda-8-get-started's order-process-boundary-events.bpmn. That source
      leaves \`errorRef\` unset (any BPMN error is meant to match); this repo's
      engine requires an explicit \`errorRef\` to deploy (see the two
      \`bpmn:error\` declarations below), so each is now pinned to its own error
      — still a one-line change from the source model, and the same
      catch-all-by-code intent. A job handler dispatched through this
      framework's normal drive loop can't trigger this path on its own
      (dispatchRound only supports complete/fail, not throwError — see
      docs/engine-coverage.md); the "Charge payment" task is wired to the
      manual force-error control in index.ts/ExampleRunner instead.
    -->
    <bpmn:boundaryEvent id="Event_1g3k0ac" name="Item not in stock" attachedToRef="Activity_0tw2fu0">
      ...
      <bpmn:errorEventDefinition id="ErrorEventDefinition_1s0nas9" errorRef="Error_OutOfStock" />
    </bpmn:boundaryEvent>
    ...
    <bpmn:boundaryEvent id="Event_0arm3xk" name="Card declined" attachedToRef="Activity_1ppsbgi">
      <bpmn:outgoing>Flow_01t7961</bpmn:outgoing>
      <bpmn:errorEventDefinition id="ErrorEventDefinition_0oh6ujx" errorRef="Error_ChargeDeclined" />
    </bpmn:boundaryEvent>
    <!--
      New addition beyond the get-started source: an interrupting timer
      boundary event on "Ship items", so this example also exercises a timer
      firing while a job is waiting — the other new-territory case this task
      calls for. \`tools/probe/fixtures/timer.bpmn\` confirms \`advanceTime\`
      fires a plain intermediate timer; boundary timers are the same
      construct attached to an activity instead of sitting inline.
    -->
    <bpmn:boundaryEvent id="Event_ShipDelayTimer" name="Shipping delayed" attachedToRef="Activity_08pg6im">
      <bpmn:outgoing>Flow_ShipDelayed</bpmn:outgoing>
      <bpmn:timerEventDefinition id="TimerEventDefinition_ShipDelay">
        <bpmn:timeDuration xsi:type="bpmn:tFormalExpression">PT8S</bpmn:timeDuration>
      </bpmn:timerEventDefinition>
    </bpmn:boundaryEvent>
    <bpmn:endEvent id="Event_ShipDelayed" name="Shipment delayed — escalated">
      <bpmn:incoming>Flow_ShipDelayed</bpmn:incoming>
    </bpmn:endEvent>
    <bpmn:sequenceFlow id="Flow_ShipDelayed" sourceRef="Event_ShipDelayTimer" targetRef="Event_ShipDelayed" />
  </bpmn:process>
  <bpmn:error id="Error_OutOfStock" name="Item not in stock" errorCode="OUT_OF_STOCK" />
  <bpmn:error id="Error_ChargeDeclined" name="Card declined" errorCode="CHARGE_DECLINED" />
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    <bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="order-process-boundary-events">
      ...
      <bpmndi:BPMNShape id="Event_14i8hvp_di" bpmnElement="Event_0arm3xk">
        ...
        <bpmndi:BPMNLabel>
          <dc:Bounds x="475" y="185" width="70" height="14" />
        </bpmndi:BPMNLabel>
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Event_ShipDelayTimer_di" bpmnElement="Event_ShipDelayTimer">
        <dc:Bounds x="652" y="142" width="36" height="36" />
        <bpmndi:BPMNLabel>
          <dc:Bounds x="630" y="185" width="80" height="27" />
        </bpmndi:BPMNLabel>
      </bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="Event_ShipDelayed_di" bpmnElement="Event_ShipDelayed">
        <dc:Bounds x="922" y="242" width="36" height="36" />
        <bpmndi:BPMNLabel>
          <dc:Bounds x="899" y="285" width="82" height="27" />
        </bpmndi:BPMNLabel>
      </bpmndi:BPMNShape>
      ...
      <bpmndi:BPMNEdge id="Flow_ShipDelayed_di" bpmnElement="Flow_ShipDelayed">
...
}`,r={...n,bpmn:e,seed:{item:"camunda-t-shirt",quantity:3},handlers:[{elementId:"Activity_0tw2fu0",standsInFor:"job worker — check-inventory",source:i},{elementId:"Activity_1ppsbgi",standsInFor:"job worker — charge-payment",source:t,manualControl:{label:"Charge payment method",completeLabel:"✅ Complete normally",action:{kind:"error",errorCode:"CHARGE_DECLINED",message:"The card issuer declined the charge.",label:"❌ Simulate: card declined"}}},{elementId:"Activity_08pg6im",standsInFor:"job worker — ship-items",source:o,manualControl:{label:"Ship items",completeLabel:"✅ Ship it",action:{kind:"timer",label:"🕐 Fire the shipping-delayed timer"}}}]};export{r as orderProcessBoundaryEvents};
