...
const learnSignalBroadcast: ExampleDef = {
  ...
  group: "learn-bpmn",
  blurb:
    "A signal intermediate catch event parks the token until someone broadcasts a signal by name. Unlike a message, a signal isn't correlated to one waiting instance — broadcasting it unblocks every open subscription for that name, across every running instance, at once. Run this and watch the token park at \"Wait for all-clear\" the moment the process starts; the page then broadcasts the all-clear signal for you, which unblocks the wait, runs \"Resume operations\", and reaches the end event. To see the name being matched: in the Code panel, open the model tab, click \"Wait for all-clear\", and expand Signal in the properties panel on the right — Name holds `all-clear`. Rename it and the run still works, because this page broadcasts whatever name the subscription is waiting on; in a real deployment the broadcaster is a separate actor, so a name only this side knows about is never matched and the token parks forever.",
