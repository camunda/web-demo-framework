...
const learnMultiInstanceParallel: ExampleDef = {
  ...
  group: "learn-bpmn",
  blurb:
    "A multi-instance activity runs its task once per element of a collection, spawning that many job instances of the same element in parallel, and only lets the token move on once every one of them completes. Run this and watch three 'Process item' jobs activate together for apple, banana, cherry, and complete (in any order) before the process reaches its end event. The whole construct hinges on one property: in the Code panel, open the model tab, click \"Process item\", and expand Multi-instance in the properties panel on the right — Input collection holds `items`, the FEEL expression naming the variable to fan out over (the Start panel supplies it). Point it at a different variable, or add a fourth item to the Start payload, and hit Run to watch the number of parallel jobs follow. Drop the input collection entirely and the activity silently degenerates to a single ordinary instance, with no error to tell you it happened. Revert to original puts it back.",
