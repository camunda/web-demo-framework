...
| **Endpoint** | Any OpenAI-compatible server, in two modes: a local **Ollama**, or a **remote provider URL + API key**. | The Ollama mode is offered **only when the page itself is served from localhost** — Ollama's CORS allowlist covers localhost origins only, so a tunnelled or deployed page is refused; the app detects this and says so instead of blaming the server. A hosted page is offered the provider-URL mode instead, with the key held in memory for the tab only |
